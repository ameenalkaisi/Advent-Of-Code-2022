...
fn main() {
    ...
    let first_div_packet = Packet::List(vec![Packet::List(vec![Packet::Value(2)])]);
    let second_div_packet = Packet::List(vec![Packet::List(vec![Packet::Value(6)])]);
    packets.push(first_div_packet.clone());
    packets.push(second_div_packet.clone());

    packets.sort();

    let first_index = packets.iter().position(|packet| packet == &first_div_packet).unwrap() + 1;
    let second_index = packets.iter().position(|packet| packet == &second_div_packet).unwrap() + 1;

    println!("indices sum: {}", first_index * second_index);
}
