...
fn floyd_warshall(cave_system: &CaveSystem) -> HashMap<(String, String), i64> {
    ...
    // initalize every LABEL X LABEL pair into 0
...
fn all_relief_paths(
    ...
    for valve in valves {
        if !used.contains(valve) {
            ...
            // ONLY go into the branch if you know the distance to get there
            // is movable
...
fn main() -> Result<()> {
    let cave_system = read(BufReader::new(
        std::fs::File::open(env::args().nth(1).unwrap()).unwrap(),
    ))?;
