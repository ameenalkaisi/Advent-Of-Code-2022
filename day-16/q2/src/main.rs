...
fn find_all_paths(
    start: &String,
    valves: &Vec<String>,
    valve_map: &HashMap<String, ValvePtr>,
    path_used: i64,
    ...
    let pressure_relieved =
        (total_time - (path_used + 1)) * valve_map.get(start).unwrap().borrow().flow_rate;
    ...
    for valve in valves {
        if !used.contains(valve) {
            ...
            // end point is always "AA"
            // find other valves so that K <- valve <- "AA"
...
fn main() {
    ...
    // filter nonzero ones
    let mut meaningful_valves: Vec<String> = vec![];
    all_valves.keys().for_each(|label| {
        if all_valves.get(label).unwrap().borrow().flow_rate > 0 {
            meaningful_valves.push(label.clone());
        }
    });

    let mut paths = HashMap::new();
    let distances = find_all_distances(&all_valves);
    meaningful_valves.iter().for_each(|valve| {
        find_all_paths(
            valve,
            &meaningful_valves,
            &all_valves,
            // starts off as path "valve" -> "AA", with distance from the distances arr
            distances
