...
fn main() {
    ...
    for line in lines {
        ...
        // get the ranges
        // it seems that the regex takes a while to compute
        ...
        for cap in regex.captures_iter(&line) {
            ...
            // one of the range's beginning MUST be contained in the other container
            // for it to be true
            if (first_begin <= second_begin && second_begin <= first_end)
                || (second_begin <= first_begin && first_begin <= second_end)
            {
