...
fn main() {
    ...
    println!(
        "{}",
        get_smallest_dir_that_would_free_space(
            root.clone(),
            30_000_000 - (70_000_000 - root.clone().borrow().size.unwrap())
        )
    );
}

fn get_smallest_dir_that_would_free_space(root: Rc<RefCell<Dir>>, needed_size: u32) -> u32 {
    let root = root.clone();
    let cur: &Dir = &root.borrow();

    let mut best = cur.size.unwrap();
    for i in cur.subdirs.iter() {
        let subdir_result = get_smallest_dir_that_would_free_space(i.clone(), needed_size);
        if best < needed_size {
            best = subdir_result
        } else if subdir_result < best && subdir_result >= needed_size {
            best = subdir_result
        }
    }
